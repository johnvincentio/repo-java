...
public class MovieTextInfo extends MovieTimeBlocks {
	...
	protected void drawInfo(PdfContentByte directcontent) {
		...
		for (int i = 0; i < LOCATIONS; i++) {
			...
			directcontent.showTextAligned(Element.ALIGN_CENTER, locations
					.get(i), x, y, 90);
		}
	...
	protected void drawDateInfo(Date day, int d, PdfContentByte directcontent) {
		...
		x = OFFSET_LEFT + WIDTH;
		directcontent.showTextAligned(Element.ALIGN_RIGHT, day.toString(), x, y, 0);
		directcontent.endText();
	}
	...
	protected void drawMovieInfo(Screening screening,
		...
		if (screening.isPress()) {
			...
			ColumnText.showTextAligned(directcontent, Element.ALIGN_CENTER,
					press, (rect.getLeft() + rect.getRight()) / 2, rect
							.getBottom()
							+ rect.getHeight() / 4, 0);
		}
	}

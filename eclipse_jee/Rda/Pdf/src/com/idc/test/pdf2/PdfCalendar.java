...
public class PdfCalendar {
	private static final String OUTPUT_PDF = "C:/irac7/wrkspc/Rda/Pdf/test/pdf2/PdfCalendar.pdf";

	/** The resulting PDF file. */
	public static final String RESULT = OUTPUT_PDF;
	...
	public PdfCalendar() throws DocumentException, IOException {
		// fonts
		BaseFont bf_normal = BaseFont.createFont("c://windows/fonts/arial.ttf",
				BaseFont.WINANSI, BaseFont.EMBEDDED);
		normal = new Font(bf_normal, 16);
		small = new Font(bf_normal, 8);
		BaseFont bf_bold = BaseFont.createFont("c://windows/fonts/arialbd.ttf",
				BaseFont.WINANSI, BaseFont.EMBEDDED);
	 ...
	 * @param locale
	 *            Locale in case you want to create a Calendar in another
	 *            language
	 * @param year
	...
	public void createPdf(String filename, Locale locale, int year)
		...
		for (int month = 0; month < 12; month++) {
			...
			table.writeSelectedRows(0, -1, 169, table.getTotalHeight() + 18,
					canvas);
			document.newPage();
		}
		// step 5
	 ...
	 * @throws IOException
	 */
	public void drawImageAndText(PdfContentByte canvas, Calendar calendar)
		...
		Phrase p = new Phrase(String.format("%s - \u00a9 Katharine Osborne",
	...
	public PdfPCell getMonthCell(Calendar calendar, Locale locale) {
		...
		Paragraph p = new Paragraph(String.format(locale, "%1$tB %1$tY",
				calendar), bold);

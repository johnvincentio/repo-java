...
public class MakeSQL {
	...
	private static final boolean bInserts = false;
	private static final boolean bTables = true;

	public static void main(String[] args) {
		MakeSQL test = new MakeSQL();
		test.doWork();
	}
	...
	public void doWork() {
		...
		ArrayList<String> includeTableList = new ArrayList<String>();
		ArrayList allTables = db.getTables (includeTableList);
 ...
 */
				...
				addMsgNL();
				addMsgNL ("drop table "+schema+"."+tableInfo.getName()+";");
/*
 * handle create table
 */
				LogHelper.info("Writing the create table statements for table "+tableInfo.getName());
				addMsgNL();
				addMsgNL ("create table "+schema+"."+tableInfo.getName()+" (");
 ...
 */
				...
				buf = null;
				buf = new StringBuffer();
				buf.append ("grant select, insert, update, delete on table "+schema+"."+tableInfo.getName());
 ...
 */
			if (bInserts) {
				...
				rowsInfo = db.getRows (tableInfo);
//				LogHelper.info(rowsInfo.toString());
				stmt = null;
				stmt = new StringBuffer();
				stmt.append ("insert into ").append (schema).append(".").append (tableInfo.getName());
				stmt.append (" (");
				for (int j=0; j < tableInfo.getCollist().size(); j++) {
					columnInfo = (ColumnInfo) tableInfo.getCollist().get(j);

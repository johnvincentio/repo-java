...
//		String strURL = getCodeBase().getHost(); // Applet only
		...
		m_images = new ScrabbleImages(this,BOARD_SIZE);	// load letters and tiles
	...
	public void setKey(int key) {m_keyid = key;}
...
//
	private Container makeGameContentPane() {
		...
		boardPanel1.setLayout(new GridLayout(BOARD_SIZE, BOARD_SIZE, 0, 0));
		...
		m_board = new Square[BOARD_SIZE][BOARD_SIZE];
		for (int row = 0; row < BOARD_SIZE; row++) {	// add squares to board
		...
		rackPanel1.setLayout(new GridLayout(1, RACK_SIZE, 0, 0));
		m_playerRack = new Square[RACK_SIZE];
		for (int col=0; col<RACK_SIZE; col++) {
...
//
	public void sendStartMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.START);
		packet.append(-1);
	...
	private void sendInitMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.INIT);
	...
	private void sendExitMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.EXIT);
		packet.append(m_keyid);
		getMySocket().sendScrabbleMessage(packet.getString());
		Debug.println("<<< sendExitMessage");
	}
	private void sendNameMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.NAME);
		packet.append(-1);
	...
	private void sendClearMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.CLEAR);
		packet.append(m_keyid);
	...
	private void sendMoveMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.MOVE);
		packet.append(m_keyid);
	...
	private void sendPassMessage() {
		...
		ScrabblePacketWriter packet = new ScrabblePacketWriter(PacketTypes.PASS);
		packet.append(m_keyid);
...
//
	public void processMessage(final String msg) {
		...
		ScrabblePacketReader packetReader = new ScrabblePacketReader (strTmp);

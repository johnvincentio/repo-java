...
public class ClientApp extends JFrame implements ActionListener {
	...
	private ClientImages m_images;
	private boolean m_bMyTurn = false;

	public ClientApp(String msg) {
		super(msg);
		Debug.setFile("CONSOLE",false);
		Debug.println("ClientApp()");
		m_clientMessages = new ClientMessages (this);
		String strURL = "127.0.0.1";
//		String strURL = getCodeBase().getHost(); // Applet only
		m_clientThread = new ClientThread (this, strURL); // open socket to the server
		m_images = new ClientImages(this,Constants.BOARD_SIZE);	// load letters and tiles
		...
		m_clientThread.start(); // thread to read messages from the socket
	}
	public static void main (String args[]) {	// not a lot!
		ClientApp app = new ClientApp ("Scrabble");
	}
	public ClientSocket getMySocket() {
		return m_clientThread.getSocket();
	}
...
//
	private Container makeGameContentPane() {
		...
		Debug.println(">>> ClientApp::makeContentPane()");
...
//
	public void setMessagesArea (final String msg) {
		Debug.println(">>> ClientApp::setMessagesArea()");
	...
	public void setUsernameField (final String msg) {
		Debug.println(">>> ClientApp::setUsernameField()");
		SwingUtilities.invokeLater (
			new Runnable() { 
				public void run() {
...
//
	...
	public void actionPerformed (ActionEvent e) {
		Debug.println(">>> ClientApp::actionPerformed()");

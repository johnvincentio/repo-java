...
/////////////////////////////////////////////////////////// class DragBallPanel
/**
 * When the mousePressed listener is called to position is tested to see if it's
 * in the area of the ball. If it is, (1) _canDrag is set true meaning pay
 * attention to the MouseDragged events. (2) Record where in the ball (relative
 * to the upper left coordinates) the mouse was clicked, because it looks best
 * if we drag from there.
 */
public class DragBallPanel extends JPanel implements MouseListener,
	...
	// ===================================================== method mousePressed
	/**
	 * Set _canDrag if the click is in the ball (or in the bounding box, which
	 * is lazy, but close enuf for this program). Remember displacement
	 * (dragFromX and Y) in the ball to use as relative point to display while
	 * dragging.
	 */
	public void mousePressed(MouseEvent e) {
		...
		if (x >= _ballX && x <= (_ballX + BALL_DIAMETER) && y >= _ballY
				&& y <= (_ballY + BALL_DIAMETER)) {
			_canDrag = true;
